...
clear
    ...
    # Buat script limit IP (RINGAN)
    cat > /usr/local/bin/limit-ip-cron.sh << 'EOF'
#!/bin/bash
# LIMIT IP RINGAN - VIA CRON
LOG="/var/log/limit-ip.log"
DEFAULT_LIMIT=3

if [[ -f "/etc/limit-default" ]]; then
    DEFAULT_LIMIT=$(cat /etc/limit-default)
fi
...
EOF
    ...
    # Set default limit
    echo "3" > /etc/limit-default
    
    clear
    echo -e "${GREEN}════════════════════════════════════════${NC}"
    echo -e "${GREEN}  ✅ LIMIT IP AKTIF${NC}"
    echo -e "${GREEN}════════════════════════════════════════${NC}"
    echo -e " Method : ${GREEN}Cron (Ringan)${NC}"
    echo -e " Cek    : ${YELLOW}Setiap 2 menit${NC}"
    echo -e " Log    : ${YELLOW}/var/log/limit-ip.log${NC}"
    echo -e " Default: ${YELLOW}3 IP per user${NC}"
    echo -e "${GREEN}════════════════════════════════════════${NC}"
    sleep 2
    exec bash "$0"
}
...
EOF
    ...
    echo -e "${GREEN}════════════════════════════════════════${NC}"
    echo -e " Method : ${GREEN}Cron (Ringan)${NC}"
    echo -e " Cek    : ${YELLOW}Setiap 5 menit${NC}"
    echo -e " Log    : ${YELLOW}/var/log/limit-quota.log${NC}"
    echo -e " Limit  : ${YELLOW}1GB per user${NC}"
    echo -e "${GREEN}════════════════════════════════════════${NC}"
    sleep 2
    exec bash "$0"
}
